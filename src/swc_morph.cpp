...
Segment::Segment(int start_point_idx, int end_point_type, PointR start_point,
                 PointR end_point)
    : idx(start_point_idx), type(end_point_type), start(start_point),
      end(end_point) {
...
void Segment::interpolate(std::vector<PointR> &interp_data,
      ...
      if (fields.size() == 7) {
        ...
        if (previous == -1) {
          ...
          if (previous_seg != segments.end()) {
            ...
            throw std::runtime_error("ERROR: Previous segment point " +
...
void MorphData::interpolate(std::vector<PointR> &interp_data,
                            double point_distance) {
