...
double distsqFromSurf(const Point &p, std::vector<PointR> &interpolation,
                      bgi::rtree<sp_value, bgi::quadratic<16>> &rtree,
                      double search_range) {
  sp_box query(sp_point(p.x() - search_range, p.y() - search_range,
                        p.z() - search_range),
               sp_point(p.x() + search_range, p.y() + search_range,
                        p.z() + search_range));
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    desc.add_options()("help,h", "Help message")(
        ...
        "surfmesh", po::bool_switch()->default_value(false),
        "Also output surface mesh")(
        "interp-distance", po::value<double>()->default_value(0.05f, "0.05"),
        "Distance between two interpolation points")(
        "fc-angle", po::value<double>()->default_value(25.0f, "25.0"),
        "Facet criteria - Angle")(
        "fc-size", po::value<double>()->default_value(1.0f, "1.0"),
        "Facet criteria - Size")(
        "fc-distance", po::value<double>()->default_value(0.1f, "0.1"),
        "Facet criteria - Distance")(
        "cc-ratio", po::value<double>()->default_value(2.0f, "2.0"),
        "Cell criteria - Cell radius edge ratio")(
        "cc-size", po::value<double>()->default_value(1.0f, "1.0"),
    ...
    if (vm.count("help")) {
      std::cout << "Usage: ./SWCTetMesher input [options]\n";
      std::cout << desc;
      return 0;
    }

    if (vm.count("input")) {
      std::cout << "Input SWC file is: " << vm["input"].as<std::string>()
                << "\n";
    } else {
    ...
    if (interp_distance > length_max / 2) {
      std::cerr << "WARNING: Interpolation distance is larger than half of the "
    ...
    std::cout << "Number of spheres used for interpolation: "
              << interpolation.size() << "\n";
    ...
    for (auto i = 0; i < n_points; i++) {
      ...
      sp_box sp_b(sp_point(b.x_min, b.y_min, b.z_min),
                  sp_point(b.x_max, b.y_max, b.z_max));
    ...
    Mesh_domain domain = Mesh_domain::create_implicit_mesh_domain(
        boundary_func,
        CGAL::Bbox_3(morph_bbox.x_min, morph_bbox.y_min, morph_bbox.z_min,
                     morph_bbox.x_max, morph_bbox.y_max, morph_bbox.z_max));

    std::cout << "\nMeshing...\n";
    Facet_criteria facet_criteria(vm["fc-angle"].as<double>(),
                                  vm["fc-size"].as<double>(),
                                  vm["fc-distance"].as<double>());
    Cell_criteria cell_criteria(vm["cc-ratio"].as<double>(),
                                vm["cc-size"].as<double>());
